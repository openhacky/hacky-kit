...
public enum HackyActivationV01 {
    ...
    private static func japanesePrefix(name: String, turns: Int) -> String {
        ...
        return "\(activationMarker)\n"
            ...
            + "この効果はAIの返信\(turns)回で終了します。適用後の最初の返信を1回目として数えます。\(counter)最終回は効果に沿った本文の後、独立した最終行に「Hackyの効果がなくなりました。」と表示し、それ以降は効果を適用しないでください。ユーザーが停止を明示した場合は直ちに終了し、同じ終了文を一度だけ表示してください。\n\n"
